...
import { Code2, Smartphone, Server, Cloud, Database, Cpu, ChevronRight } from 'lucide-react';
...
const expertiseGroups = [
  ...
  },
  {
    ...
    technologies: ['FastAPI', 'Laravel', 'Python', 'PHP'],
  },
  {
    icon: Database,
    label: 'Data',
    description: 'Stockage et accès aux données',
  ...
  },
  {
    ...
    technologies: ['Docker', 'Nginx', 'GitHub Actions', 'VPS'],
  },
  {
    icon: Cpu,
    label: 'Architecture',
    description: 'Conception et patterns logiciels',
    technologies: ['Clean Architecture', 'Microservices', 'API REST', 'SaaS'],
  },
];
              ...
              <Reveal key={group.label} delay={i * 0.05}>
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className={`text-base font-bold transition-colors ${active === i ? 'text-white' : 'text-ink-200 group-hover:text-white'}`}>
                      {group.label}
                    </div>
                  ...
                  <ChevronRight
                    className={`h-4 w-4 flex-shrink-0 transition-all duration-300 ${
                      active === i ? 'text-primary-400 translate-x-0' : 'text-ink-600 -translate-x-1 opacity-0 group-hover:opacity-100 group-hover:translate-x-0'
                    }`}
                  />
                </button>
              </Reveal>
                  ...
                  <div className="relative">
                    <div className="flex items-center gap-3 mb-2">
                      <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-primary-500/20 text-primary-400">
                        {(() => {
                          const Icon = expertiseGroups[active].icon;
                          return <Icon className="h-5 w-5" />;
                        })()}
                      </div>
                      <div>
                        <div className="text-xl font-bold text-white">{expertiseGroups[active].label}</div>
                        <div className="text-sm text-ink-400">{expertiseGroups[active].description}</div>
                      </div>
                    </div>

                    <div className="mt-6 flex flex-wrap gap-2.5">
                      {expertiseGroups[active].technologies.map((tech, i) => (
                        <motion.span
                          key={tech}
                    ...
                    <div className="mt-8 pt-6 border-t border-white/10 flex items-center gap-6">
                      <div>
                        <div className="text-2xl font-extrabold text-white font-display">{expertiseGroups[active].technologies.length}+</div>
