  ...
  if (view === 'edit') {
    return (
      <div className="max-w-2xl mx-auto">
        ...
        <h2 className="text-2xl font-bold text-ink-900 mb-6">{editing ? 'Modifier la valeur' : 'Nouvelle valeur'}</h2>
        <form onSubmit={handleSave} className="space-y-5">
          <div className="rounded-2xl border border-ink-100 bg-white p-6 space-y-5">
            ...
            <div><label className="block text-sm font-medium text-ink-700 mb-2">Titre *</label><input type="text" required value={formData.title} onChange={(e) => setFormData({ ...formData, title: e.target.value })} placeholder="Excellence" className={inputClass} /></div>
            <div><label className="block text-sm font-medium text-ink-700 mb-2">Description *</label><textarea required rows={3} value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })} placeholder="Description de la valeur" className={inputClass.replace('h-11', '')} /></div>
          </div>
